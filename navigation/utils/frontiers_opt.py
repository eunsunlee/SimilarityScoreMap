...
def get_frontiers(ref_map): 
    ...
    for i in range(num_envs): 
        ...
        frontier_list.append(centroids[1:].astype(int))
        frontier_stats.append(stats[1:])


    return frontier_list, frontier_stats, obstacle_map, obstacle_region, explored_map, frontier #exclude background


def get_frontiers_multi(args, ref_map, freq_map, frontier_filter, map_combined_envs): 
    num_envs = ref_map.shape[0] 
    obstacle_boundary = 20 
    unit_size_cm = 10 
    frontier_list = []
    ...
    if np.max(map_combined_envs[1:]):
        combined_freq_map = torch.zeros(freq_map[0][0,1].shape)
        for t in range(num_envs): 
            if map_combined_envs[t]: 
                combined_freq_map = combined_freq_map + freq_map[t][0,1].detach().cpu()
    ...
    for i in range(num_envs): 
        ...
        for f in range(num_labels): 
            if f != 0: 
                ...
                if np.max(map_combined_envs[1:]) and map_combined_envs[i]: 
                    frontier_w_freq = combined_freq_map.cpu().numpy().copy()
                else:
    ...
    return frontier_list,frontier_stats, obstacle_map, obstacle_region, explored_map, frontier_img, filtered_avg_freq_vals_list  #exclude background


def get_frontier_based_global_goals_multi_opt( args, temp_global_goals, frontiers, stats, full_pose, map_combined, filtered_avg_freq_vals_list, least_sim_node_poses, map_combined_envs, full_map, frontier_img): 
    optimized_env =  np.where(np.array(map_combined_envs) == 1)[0].tolist()
...
def get_frontier_based_global_goals(frontiers, full_pose, map_combined): 
