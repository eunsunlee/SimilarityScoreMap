...
class Exploration_Env_hybrid(habitat.RLEnv):
    ...
    def __init__(self, args, rank, config_env, config_baseline, dataset):
        ...
        HabitatSimActions.extend_action_space("EXTRA1")
        HabitatSimActions.extend_action_space("EXTRA2")
        HabitatSimActions.extend_action_space("EXTRA3")

        HabitatSimActions.extend_action_space("EXTRA4")
        HabitatSimActions.extend_action_space("EXTRA5")
        HabitatSimActions.extend_action_space("EXTRA6")

        HabitatSimActions.extend_action_space("EXTRA7")
        HabitatSimActions.extend_action_space("EXTRA8")
        HabitatSimActions.extend_action_space("EXTRA9")

        HabitatSimActions.extend_action_space("EXTRA10")
        HabitatSimActions.extend_action_space("EXTRA11")
        HabitatSimActions.extend_action_space("EXTRA12")
        ...
        if args.noisy_actions:   
            ...
            config_env.SIMULATOR.NOISE_MODEL = habitat.Config()
    ...
    def reset(self):
        ...
        while self.explorable_map is None:
            ...
            self.explorable_map, temp_info = self._get_gt_map(full_map_size)
        ...
        self.scene_name = self._env.current_episode.scene_id
        ...
        self.info = {
            ...
            'start_position': self._env.current_episode.start_position,
    ...
    def step(self, action):
        ...
        self.noise_level = current_agent_no/self.args.num_processes
        ...
        return state, depth, rew, done, self.info
    ...
    def _get_gt_map(self, full_map_size):
        ...
        rot_mat_inv, trans_mat_inv = get_grid(-st, (1, 1, grid_size, grid_size), torch.device("cpu"))
        rotated_inv = F.grid_sample(rotated, rot_mat_inv)
        translated_inv = F.grid_sample(translated, trans_mat_inv)        
        ...
        if full_map_size > grid_size:
            episode_map[(full_map_size - grid_size)//2:
                        ...
                        (full_map_size - grid_size)//2 + grid_size] = \
                                rotated[0,0]
            boundary = [0, (full_map_size - grid_size)//2, (full_map_size - grid_size)//2 + grid_size, (full_map_size - grid_size)//2, (full_map_size - grid_size)//2 + grid_size ]
            ...
            episode_map = rotated[0,0,
                              ...
                              (grid_size - full_map_size)//2:
                              (grid_size - full_map_size)//2 + full_map_size]
            boundary = [1, (grid_size - full_map_size)//2, (grid_size - full_map_size)//2 + full_map_size,  (grid_size - full_map_size)//2,(grid_size - full_map_size)//2 + full_map_size ]
        ...
        temp_info = {
                ...
                'min_x': min_x, 
                'min_y': min_y, 
                'sim_location': self.get_sim_location(),
                'range_x': range_x, 
                'range_y': range_y, 
                'map_obj_max': self.map_obj.max, 
                'map_obj_origin': self.map_obj.origin
                }

        return episode_map, temp_info
