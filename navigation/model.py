...
# Neural SLAM Module code
class Neural_SLAM_Module_FM(nn.Module):
    """
    """

    def __init__(self, args):
        super(Neural_SLAM_Module_FM, self).__init__()
    ...
    def forward(self, obs_last, obs, poses, maps, explored, current_poses,
        ...
        if build_maps:
            ...
            with torch.no_grad():
                ...
                # plt.imsave("temp.png", rotated[0,0].cpu().numpy())
                # plt.imsave("temp0.png", maps[0].cpu().numpy())
        else:
            map_pred = None
            exp_pred = None
            current_poses = None
